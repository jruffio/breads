...
def iso_hpffm(nonlin_paras, cubeobj, planet_f=None, transmission=None,boxw=1, psfw=1.2,res_hpf=50,badpixfraction=0.75):
    """
    ...
    Args:
        ...
        res_hpf: float, resolution of the continuum to be subtracted.
    ...
    cube_stamp = _paddata[:, k - w:k + w + 1, l - w:l + w + 1]
    badpix_stamp = _padbad_pixels[:, k - w:k + w + 1, l - w:l + w + 1]
    ...
    if np.size(where_finite[0]) <= (1-badpixfraction) * np.size(d):
        ...
        scaled_psfs_lpf = np.zeros((nz,boxw,boxw))+np.nan
        for _k in range(boxw):
            for _l in range(boxw):
                lwvs = wvs[:,np.clip(k-2*w+_k,0,nywv-1),np.clip(l-2*w+_l,0,nxwv-1)]
                scaled_psfs_lpf[:,_k,_l] = broaden(lwvs,scaled_psfs[:,_k,_l]*badpix_stamp[:,_k,_l],res_hpf)
                scaled_psfs_hpf[:,_k,_l] = scaled_psfs[:,_k,_l]-scaled_psfs_lpf[:,_k,_l]
