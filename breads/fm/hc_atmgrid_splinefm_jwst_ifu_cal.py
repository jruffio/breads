...
def hc_atmgrid_splinefm_jwst_ifu_cal(nonlin_paras, cubeobj, atm_grid=None, atm_grid_wvs=None, star_func=None,radius_as=0.2, nodes=20,
    ...
    Args:
        ...
        return_extra_outputs: bool (default is False)
            if True returns some optional outputs.
            extra_outputs["wvs"] = w
            extra_outputs["ras"] = x
        ...
        detec_KLs: None
        wvs_KLs_f: None
        regularization: Can be "default", "user" or None.
    ...
    fitback = False #TODO: what should we do about this fitback variable? Hardcoding here just for old degugging purposes? if we remove it a lot of the code can be cleaned up...
    N_linpara = 1
    ...
    if np.size(where_trace_finite[0]) <= (1 - badpixfraction) * np.sum(new_mask * larger_mask_comp) or vsini < 0:
        ...
        if regularization == "user":
            ...
            rows_reg_speckles = np.tile((np.pad(rows_ids, (0, Nrows - np.size(rows_ids)), constant_values=0))[:, None],
                                        (1, N_nodes))
        ...
        if regularization == "default":
            ...
            if return_extra_outputs:
                ...
                # TODO I'm not sure to understand this exception, does it mean that we should not use "default" regularization?
                # TODO Should we remove this input then? Or fix it?
...
def hc_atmgrid_splinefm_jwst_ifu_cal_cleaned(nonlin_paras, cubeobj, atm_grid=None, atm_grid_wvs=None, star_func=None,radius_as=0.2, nodes=20,
