...
def hc_atmgrid_splinefm_jwst_miri_cal_old(nonlin_paras, cubeobj, channel='CH2', atm_grid=None, atm_grid_wvs=None, star_func=None,radius_as=0.2, nodes=20,
             badpixfraction=0.75,fix_parameters=None,Nrows_max=200,return_extra_outputs=False,detec_KLs=None,wvs_KLs_f=None,
             regularization=None,reg_mean_map=None,reg_std_map=None, debug=False):
    ...
    """
    print("Using hc_atmgrid_aplinefm_jwst_miri_cal function for forward modeling")
    extra_outputs = {}
    ...
    if debug:
        print(f"[DEBUG] planet model {planet_model}")
    if np.sum(planet_model)==0 or np.size(atm_grid_wvs) != np.size(planet_model):
        print("Wrong planet model for input") 
    ...
    cols_ids = np.where(mask_vec)[0]

    new_mask = np.tile(mask_vec,(ny, 1))
    ...
    if debug:
        print(f"[DEBUG] d {len(np.where(np.isfinite(data))[0])}, bp {len(np.where(np.isfinite(bad_pixels))[0])}, noise {len(np.where(noise!=0)[0])}, comp {len(np.where(np.isfinite(comp_spec))[0])}, star {len(np.where(np.isfinite(star_func(wvs)))[0])}")
    Ncols = np.size(cols_ids)
    if debug: 
        print(f"[DEBUG] number of columns {Ncols}, col_min {np.nanmin(cols_ids)}, col_max {np.nanmax(cols_ids)}")
    Nd = np.size(where_trace_finite[1])
    if Ncols > Nrows_max:
    ...
    if vsini < 0:
        print("vsini<0, returning empty arrays")
        return np.array([]), np.array([]).reshape(0, N_linpara), np.array([])

    if np.size(where_trace_finite[0]) <= (1-badpixfraction) * np.sum(new_mask*larger_mask_comp):
        print("No trace found or vsini<0, returning empty arrays")
        print(f"[DEBUG] np.size(where_trace_finite[0]) {np.size(where_trace_finite[0])}")
        print(f"[DEBUG] 1-badpixfraction {1-badpixfraction}, np.sum(new_mask*larger_mask_comp) {np.sum(new_mask*larger_mask_comp)}")
        return np.array([]), np.array([]).reshape(0,N_linpara), np.array([])
    else:
        M_speckles = np.zeros((Nd, Ncols, N_nodes))
        if regularization == "user":
            d_reg_speckles = np.zeros((Ncols, N_nodes)) + np.nan
            s_reg_speckles = np.zeros((Ncols, N_nodes)) + np.nan
            wvs_reg_speckles = np.zeros((Ncols, N_nodes)) + np.nan
            rows_reg_speckles = np.tile((np.pad(cols_ids,(0,Ncols-np.size(cols_ids)),constant_values=0))[:,None],(1,N_nodes))
        if wvs_KLs_f is not None:
            M_KLs = np.zeros((Nd, Ncols, len(wvs_KLs_f)))
        if detec_KLs is not None:
            M_KLs_detec = np.zeros((Nd,Ncols, detec_KLs.shape[1]))
        for _k in range(Ncols):
            where_finite_and_in_row = np.where(where_trace_finite[0]==cols_ids[_k])
            if np.size(where_finite_and_in_row[0]) == 0:
                continue
            M_spline = get_spline_model(x_nodes, wvs[cols_ids[_k], :], spline_degree=3)
            ...
            if regularization == "user":
                d_reg_speckles[_k,:] = reg_mean_map[cols_ids[_k],:]
                d_reg_speckles[_k,where_del_col[0]] = np.nan
                s_reg_speckles[_k,:] = reg_std_map[cols_ids[_k],:]
        ...
        M_speckles = np.reshape(M_speckles, (Nd, Ncols * N_nodes))
        if fitback:
            M_background = M_speckles
        M_speckles = M_speckles*star_func(w)[:,None]
        print("[DEBUG] star_func values:", np.nanmax(star_func(w)), np.nanmin(star_func(w)))
        if wvs_KLs_f is not None:
            M_KLs = np.reshape(M_KLs, (Nd, Ncols * len(wvs_KLs_f)))
        if detec_KLs is not None:
            M_KLs_detec = np.reshape(M_KLs_detec, (Nd, Ncols * detec_KLs.shape[1]))
        ...
        if debug:
            print("[DEBUG] extra_outputs", extra_outputs)
            print("[DEBUG] len extra_outputs", len(extra_outputs)) 
...
def hc_atmgrid_splinefm_jwst_miri_cal(nonlin_paras, cubeobj, channel="2", atm_grid=None, atm_grid_wvs=None, star_func=None,radius_as=0.2, nodes=20,
             badpixfraction=0.75,fix_parameters=None,Nrows_max=200,return_extra_outputs=False,detec_KLs=None,wvs_KLs_f=None,
             regularization=None,reg_mean_map=None,reg_std_map=None, debug=True):
    ...
    planet_model = atm_grid(atm_paras)[0]
    if np.sum(planet_model)==0 or np.size(atm_grid_wvs) != np.size(planet_model):
        return ("Something wrong in planet model")
    else:
        if vsini != 0:
            spinbroad_model = pyasl.fastRotBroad(atm_grid_wvs, planet_model, 0.1, vsini)
        else:
    ...
    where_trace_finite = np.where(new_mask*larger_mask_comp*np.isfinite(data)*np.isfinite(bad_pixels)*(noise!=0)*
                                  np.isfinite(comp_spec)*np.isfinite(star_func(wvs)))
    if debug:
        print(f"[DEBUG] d {len(np.where(np.isfinite(data))[0])}, bp {len(np.where(np.isfinite(bad_pixels))[0])}, noise {len(np.where(noise!=0)[0])}, comp {len(np.where(np.isfinite(comp_spec))[0])}")
