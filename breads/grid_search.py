...
def process_chunk(args):
    ...
    """
    nonlin_paras_list, dataobj, fm_func, fm_paras, bounds,scale_noise, marginalize_noise_scaling = args
    ...
    for k, nonlin_paras in enumerate(zip(*nonlin_paras_list)):
        try:
        # if 1:
            log_prob,rchi2,linparas,linparas_err = fitfm(nonlin_paras,dataobj,fm_func,fm_paras,bounds=bounds,
                                                                     scale_noise=scale_noise,
                                                                     marginalize_noise_scaling=marginalize_noise_scaling)
            new_N_linpara = np.size(linparas)

            if outarr_not_created:
                out_chunk = np.zeros((np.size(nonlin_paras_list[0]),1+1+2*new_N_linpara))+np.nan
                outarr_not_created = False
            old_N_linpara = int((out_chunk.shape[-1] - 2) / 2)
            ...
            if old_N_linpara >= new_N_linpara:
                ...
                out_chunk[k,1] = rchi2
                out_chunk[k,2:2 + new_N_linpara] = linparas
                out_chunk[k,2 + old_N_linpara:2 + old_N_linpara + new_N_linpara] = linparas_err
                ...
                list2concatenate = []
                list2concatenate.append(out_chunk[:,0:2 + old_N_linpara])
                list2concatenate.append(np.zeros(new_out_shape))
                list2concatenate.append(out_chunk[:,2 + old_N_linpara::])
                list2concatenate.append(np.zeros(new_out_shape))
                out_chunk = np.concatenate(list2concatenate, axis=1)
                out_chunk[k,0] = log_prob
                out_chunk[k,1] = rchi2
                out_chunk[k,2:2 + new_N_linpara] = linparas
                out_chunk[k,2 + new_N_linpara::] = linparas_err
...
def grid_search(para_vecs,dataobj,fm_func,fm_paras,numthreads=None,bounds=None,scale_noise=True,marginalize_noise_scaling=False):
    """
    ...
    ----------
        para_vecs : list
            [vec1,vec2,...] List of 1d arrays defining the sampling of the grid of non-linear parameters such as
            rv, y, x. The meaning and number of non-linear parameters depends on the forward model defined.
        dataobj : Data object
            A data object of type breads.instruments.instrument.Instrument to be analyzed.
        fm_func : Function
        ...
        numthreads : int
            Number of processes to be used in parallelization. Non parallization if defined as None (default).
        bounds : Tuple
    ...
    if numthreads is None:
        _out = process_chunk((para_grids,dataobj,fm_func,fm_paras,bounds,scale_noise,marginalize_noise_scaling))
        ...
        for k,(indices, output_list) in enumerate(zip(indices_lists,output_lists)):
            ...
            for l,outvec in zip(indices,output_list):
                ...
                if out.shape[-1] >= np.size(output_list[0]):
                    # If the out array has more parameters than the current fit
                    old_N_linpara = int((out.shape[-1]-2)/2)
                    new_N_linpara = int((np.size(outvec)-2)/2)
                    out[np.unravel_index(l, out_shape[0:len(para_vecs)])][0:2+new_N_linpara] = outvec[0:2+new_N_linpara]
                    out[np.unravel_index(l, out_shape[0:len(para_vecs)])][2+old_N_linpara:2+old_N_linpara+new_N_linpara] = outvec[2+new_N_linpara::]
                    ...
                    out = out.T
                    list2concatenate = []
                    list2concatenate.append(out[0:2+old_N_linpara])
                    list2concatenate.append(np.zeros(new_out_shape).T)
                    list2concatenate.append(out[2+old_N_linpara::])
                    list2concatenate.append(np.zeros(new_out_shape).T)
    ...
    N_linpara = int((out.shape[-1]-2)/2)
    out = np.moveaxis(out, -1, 0)
    log_prob = out[0]
    rchi2 = out[1]
    linparas = np.moveaxis(out[2:2+N_linpara], 0, -1)
    linparas_err = np.moveaxis(out[2+N_linpara:2+2*N_linpara], 0, -1)
