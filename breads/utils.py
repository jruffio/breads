...
def _task_findbadpix(paras):
    ...
    for k in range(data_arr.shape[1]):
        where_data_finite = \
            np.where(np.isfinite(badpix_arr[:, k]) * np.isfinite(data_arr[:,k]) * \
                np.isfinite(noise_arr[:, k]) * (noise_arr[:,k]!=0))
        ...
        where_bad = np.where((np.abs(res[:, k]) > 3 * \
            median_absolute_deviation(res[where_data_finite[0], k])) | np.isnan(res[:, k]))
        ...
        new_data_arr[:, k] = np.array(pd.DataFrame(new_data_arr[:, \
                k]).interpolate(method="linear").fillna(method="bfill").fillna(method="ffill"))[:, 0]
...
def _remove_edges(paras):
    ...
    for slice in cp_slices:
        ...
        slice[np.where(np.isnan(correlate2d(slice, \
            np.ones((nan_mask_boxsize,nan_mask_boxsize)), mode="same")))] = np.nan
...
def findbadpix(data: Instrument, chunks=20, mypool=None, med_spec=None, nan_mask_boxsize=3):
    if data.noise is None:
        noise = np.ones(data.data.shape)
    else:
        noise = data.noise
    if data.bad_pixels is None:
        bad_pixels = np.ones(data.data.shape)
    else:
        bad_pixels = data.bad_pixels

    nz, ny, nx = data.data.shape
    res = np.zeros(data.data.shape) + np.nan
    ...
    N_valid_pix = ny * nx
    if med_spec is None:
        med_spec = np.nanmedian(data.data, axis=(1, 2))
    data.bad_pixels[np.where(data.data == 0)] = np.nan

    # plt.plot(med_spec)
    # plt.show()

    if mypool is None:
        data.bad_pixels = _remove_edges((data.bad_pixels, nan_mask_boxsize))
    else:
        ...
        for k in range(N_chunks-1):
            slices_list.append(data.bad_pixels[(k*chunk_size):((k+1)*chunk_size)])
            wvs_indices_list.append(np.arange((k*chunk_size), ((k+1)*chunk_size)))
        slices_list.append(data.bad_pixels[((N_chunks-1)*chunk_size):nz])
        ...
        for indices, out in zip(wvs_indices_list,outputs_list):
            data.bad_pixels[indices, :, :] = out


    if mypool is None:
        data_list = np.reshape(data.data, (nz, nx*ny))
        noise_list = np.reshape(noise, (nz, nx*ny))
        badpix_list = np.reshape(bad_pixels, (nz, nx*ny))
        out_data, out_badpix, out_res = \
            _task_findbadpix((data_list, noise_list, badpix_list, med_spec, M_spline))
        data.data = np.reshape(out_data, (nz, ny, nx))
        data.bad_pixels = np.reshape(out_badpix, (nz, ny, nx))
        res = np.reshape(out_res, (nz, ny, nx))
    else:
        numthreads = mypool._processes
        chunk_size = N_valid_pix // (3*numthreads)
        wherenotnans = np.where(np.nansum(np.isfinite(bad_pixels),axis=0)!=0)
        ...
        for k in range(N_chunks-1):
            ...
            data_list.append(data.data[:, _row_valid_pix, _col_valid_pix])
            noise_list.append(noise[:, _row_valid_pix, _col_valid_pix])
            badpix_list.append(bad_pixels[:, _row_valid_pix, _col_valid_pix])
        ...
        for row_indices, col_indices, out in zip(row_indices_list, col_indices_list, outputs_list):
            ...
            data.data[:, row_indices, col_indices] = out_data
            data.bad_pixels[:, row_indices, col_indices] = out_badpix
...
def LPFvsHPF(myvec, cutoff):
    ...
    tmp = np.array(pd.DataFrame(np.concatenate([myvec_cp, myvec_cp[::-1]], axis=0)\
        ).interpolate(method="linear").fillna(method="bfill").fillna(method="ffill"))
    myvec_cp_lpf = np.array(pd.DataFrame(tmp).rolling(window=window, center=True).median(\
        ).interpolate(method="linear").fillna(method="bfill").fillna(method="ffill"))[0:np.size(myvec), 0]
