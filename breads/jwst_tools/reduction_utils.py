...
###########################################################################
# Functions for invoking the NIRSPEC pipeline

def run_stage1(uncal_files, output_dir, overwrite=False,maximum_cores="all", skip_dark=False):
    ...
    for i, file in enumerate(uncal_files):
        ...
        out_name = os.path.join(output_dir, os.path.basename(file).replace('uncal.fits', 'rate.fits'))
...
def run_stage2(rate_files, output_dir, skip_cubes=True, overwrite=False):
    ...
    for fid,rate_file in enumerate(rate_files):
        ...
        out_name = os.path.join(output_dir, os.path.basename(rate_file).replace('rate.fits', 'cal.fits'))
...
def run_stage1_miri(uncal_path, list_bands=None, overwrite=False, maximum_cores="1", skip_dark=False, oddeven_correction=True):
    """Run pipeline stage 1, with some customizations for reductions"""
...
def flat_fringing_stage1(uncal_path, target_name, list_bands=None, flat_path=None, flat_extended=False, overwrite=False):
    ...
    for band in list_bands:
        ...
        for fid, rate_file in enumerate(rate_files):
            ...
            best_flat, flat_name, std_min = best_flat_selection(rate_file, flat_path_rate, channel, flat_extended=flat_extended)
...
def run_stage2_miri(uncal_path, target_name, list_bands=None, custom_flatted=True, skip_cubes=True, skip_fringe=False, skip_residual_fringes=False,
...
def run_full_miri_default_pipeline(uncal_path, target_name, list_bands=None, overwrite=False):
    ...
    run_stage2_miri(uncal_path, target_name, list_bands=list_bands, custom_flatted=False, skip_cubes=False, skip_fringe=False, skip_residual_fringes=True, skip_flatfield=False, skip_straylight=False, overwrite=overwrite)
...
def run_stage_fringing_filtering(cal_files, output_dir, overwrite=False):
    """ 
    """
    ...
    f0 = 0.2
    width= 0.15
    low = (f0 - width / 2)  # Limite basse normalisée
    high = (f0 + width / 2)  # Limite haute normalisée
    b, a = butter(N=4, Wn=[low, high], btype='bandstop')

    for fid, cal_file in enumerate(cal_files):
        filename = os.path.basename(cal_file)
        with fits.open(cal_file, mode='readonly') as hdul:
            # Créer une nouvelle liste d'extensions
            new_hdulist = fits.HDUList([hdul[0]])  # Ajouter le PrimaryHDU en premier

            for i, hdu in enumerate(hdul[1:], start=1):  # Parcourir les extensions après le PrimaryHDU
                if isinstance(hdu, fits.ImageHDU):
                    if i == 1:  # Exemple : remplacer l'extension d'index 1 (2ème dans la liste)
                        cal = hdu.data
                        col_fringing = np.zeros(cal.shape[0])
                        for col_id in range(cal.shape[1]):
                            col_fringing = cal[:, col_id]
                            col_fringing[np.isnan(col_fringing)] = 0
                            cal[:, col_id] = filtfilt(b, a, col_fringing)
                            cal[:40, col_id] = 0
                            cal[-40: col_id] = 0
                        cal_filtered_files.append(cal)
                        new_image_data = cal.astype(np.float32)
                        new_hdu = fits.ImageHDU(data=new_image_data, header=hdu.header)
                        if 'EXTNAME' in hdu.header:
                            new_hdu.header['EXTNAME'] = hdu.header['EXTNAME']
                        new_hdulist.append(new_hdu)
                    else:
                        new_hdulist.append(hdu)
                else:
                    new_hdulist.append(hdu)
            new_hdulist.writeto(os.path.join(output_dir, filename), overwrite=overwrite)
            print(f"Filtered cal files saved to {os.path.join(output_dir, filename)}")
    return cal_filtered_files
...
def run_stage1_miri_brandt(uncal_files, output_dir, overwrite=False, maximum_cores="all"):
    """ Run pipeline stage 1, with some customizations for reductions
    intended to be used with breads for IFU high contrast
    ...
    for i, file in enumerate(uncal_files):
        ...
        step_parameters = {
            # group_scale - run with defaults
            # dq_init - run with defaults
            # emicorr - run with defaults
            'saturation': {'n_pix_grow_sat': 0},  # check for saturated pixels, but do not expand to adjacent pixels
            # firstframe - run with defaults
            # lastframe - run with defaults
            # reset - run with defaults
            # linearity - run with defaults
            #'rscd': {'skip': True},
            #'dark_current': {'skip': True},
            #'refpix': {'skip': True},
            #'jump': {'skip': True},  # parallelize
            'ramp_fit': {'algorithm': 'LIKELY', 'maximum_cores': maximum_cores},  # parallelize
...
def run_coordinate_recenter_miri(cal_files, utils_dir,crds_dir, init_centroid = (0,0),wv_sampling=None, N_wvs_nodes=40,
    ...
    _wv_min = regwvs_combdataobj.wv_sampling[0]+0.1*(regwvs_combdataobj.wv_sampling[-1]-regwvs_combdataobj.wv_sampling[0])
    _wv_max = regwvs_combdataobj.wv_sampling[-1]-0.1*(regwvs_combdataobj.wv_sampling[-1]-regwvs_combdataobj.wv_sampling[0])
    print(_wv_min, _wv_max)

    wherefinite = np.where(np.isfinite(y2fit) * (regwvs_combdataobj.wv_sampling > _wv_min) * (regwvs_combdataobj.wv_sampling < _wv_max))
    poly_p_RA = np.polyfit(x2fit[wherefinite], y2fit[wherefinite], deg=2)
    ...
    if save_plots:
        ...
        plt.plot(regwvs_combdataobj.wv_sampling,bestfit_coords[0,:,0]*1e9,linestyle="-",color=color_list[0],label="Fixed centroid",linewidth=1)
        plt.plot(regwvs_combdataobj.wv_sampling,bestfit_coords[0,:,1]*1e9,linestyle="--",color=color_list[2],label="Free centroid",linewidth=1)
        ...
        poly_model = np.polyval(poly_p_RA,regwvs_combdataobj.wv_sampling - np.nanmedian(regwvs_combdataobj.wv_sampling))
        ...
        poly_model = np.polyval(poly_p_dec,regwvs_combdataobj.wv_sampling - np.nanmedian(regwvs_combdataobj.wv_sampling))
        plt.plot(regwvs_combdataobj.wv_sampling,poly_model,label="polyfit")
...
def run_noise_clean(rate_files, stage2_dir, clean_dir, crds_dir, N_nodes = 40,model_charge_transfer=False,
    ...
    for fid,rate_file in enumerate(rate_files):
        ...
        out_name = os.path.join(clean_dir, os.path.basename(rate_file))
        cleaned_rate_files.append(out_name)
        if os.path.exists(out_name) and not overwrite:
            print(f"Output file {out_name} already exists in the cleaned output directory; skipping {rate_file}.")
            continue
...
def compute_normalized_stellar_spectrum(cal_files,utils_dir,crds_dir,coords_offset=(0,0),wv_nodes=None,
    ...
    for filename in cal_files:
        ...
        preproc_task_list = []
        preproc_task_list.append(["compute_med_filt_badpix", {"window_size": 50, "mad_threshold": 10}, True, True])
        preproc_task_list.append(["compute_coordinates_arrays"])
        preproc_task_list.append(["convert_MJy_per_sr_to_MJy"])
...
def compute_normalized_stellar_spectrum_miri(cal_files,channel,utils_dir,crds_dir,wave_2d,coords_offset=(0,0),wv_nodes=None,target_name=None,
                                        mask_charge_transfer_radius=None,mppool=None,
                                        ra_dec_point_sources=None,overwrite=False):
    if not os.path.exists(utils_dir):
        os.makedirs(utils_dir)
...
def compute_starlight_subtraction(cal_files,utils_dir,crds_dir,wv_nodes=None,combined_star_func=None,coords_offset=(0,0),mppool=None):
    ...
    for filename in cal_files[0::]:
        ...
        print("ICIIIIIIIIIIIIIIIIIIII", outputs)
        if outputs is None:
...
def compute_starlight_subtraction_miri(cal_files,channel,utils_dir,crds_dir,wave_2d,wv_nodes=None,combined_star_func=None,coords_offset=(0,0),mppool=None):
...
def get_combined_regwvs_miri(dataobj_list,channel,wv_sampling=None,mask_charge_transfer_radius=None,use_starsub1d = False, reload=False):
    regwvs_dataobj_list = []
    for dataobj in dataobj_list:
...
def get_2D_point_cloud_interpolator_miri(regwvs_combdataobj, wv0):
    if isinstance(regwvs_combdataobj, str):
        with fits.open(regwvs_combdataobj) as hdulist:
            data = hdulist["DATA"].data
            dra_as_array = hdulist["RA"].data
