...
class PositiveEtalonCosModel( NonLinearModel ):
    """
    Etalon Model.
    ...
    def __init__( self, copy=None, **kwargs ):
        ...
        super( PositiveEtalonCosModel, self ).__init__( 4, copy=copy, params=param,
                    names=names, **kwargs )


    def copy( self ):
        """ Copy method.  """
        return PositiveEtalonCosModel( copy=self )
    ...
    def baseResult( self, xdata, params ):
        ...
        """
        x = math.pi * xdata * params[2] * numpy.cos(params[3])
        sx = numpy.sin( x )
    ...
    def baseDerivative( self, xdata, params ):
        ...
        x = math.pi * xdata * p2 * numpy.cos(p3)
        sx = numpy.sin( x )
        cx = numpy.cos( x )
        dd = 1 + p1 * p1 * sx * sx
        dd *= dd
        return - 2 * math.pi * p0 * p1 * p1 * p2 * sx * cx / dd
    ...
    def basePartial( self, xdata, params, parlist=None ):
        ...
        dd = 1.0 / ( 1 + p1 * p1 * s2 )
        d2 = dd * dd
        partial2 = - 2 * p0 * p1 * p1 * numpy.cos(p3) * cx * sx * d2
        partial3 = 2 * p0 * p1 * p1 * p2 * numpy.sin(p3) * cx * sx * d2

        parts = { 0 : ( lambda: dd ),
                  1 : ( lambda: - 2 * p0 * p1 * s2 * d2 ),
                  2 : ( lambda: math.pi * xdata * partial2 ),
                  3 : ( lambda: math.pi * xdata * partial3 ) }
    ...
    def baseName( self ):
        ...
        """
        return ( "Etalon: f( x:p ) = p_0 / ( 1 + p_1^2 * sin^2( PI * x * p_2 * cos(p_3) ) )" )
