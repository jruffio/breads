...
class JWSTMiri_cal(Instrument):
    def __init__(self, filename=None, channel_reduction='1', utils_dir=None, save_utils=True,
        ...
        if filename is None:
            ...
            self.read_data_file(filename, channel_reduction=channel_reduction, utils_dir=utils_dir, save_utils=save_utils,
    ...
    def read_data_file(self, filename, channel_reduction='1', utils_dir=None, save_utils=True, load_utils=True,
        ...
        if self.channel_reduction not in self.channel:
            raise ValueError(f'Channel {self.channel_reduction} set for reduction but the cal files channel {self.channel} do not match')
    ...
    def compute_med_filt_badpix(self, save_utils=False, window_size=50, mad_threshold=50):
        ...
        if save_utils:
            ...
            #hdulist.append(pyfits.PrimaryHDU(data=old_badpix))
            #hdulist.append(pyfits.ImageHDU(data=old_badpix, name='SELF_BADPIX'))
    ...
    def compute_coordinates_arrays(self, save_utils=False, center_with_targname=True, targname=None):
        ...
        if center_with_targname:
            ...
            _, _, offset_ra_arcsec, offset_dec_arcsec = fit_trace(hdu, self.band_reduction_aka, self.crds_dir, everyn=10)
            hdu.close()
    ...
    def compute_webbpsf_model(self, image_mask=None, pixelscale=0.1, oversample=10, parallelize=False, wv_sampling=None, save_utils=False):
        ...
        if not parallelize:
            ...
            for wv_id, wv in enumerate(wv_sampling):
                print(f"Current index of wavelength {wv_id}, Current wavelength {wv}, Total number of wavelength {nwavelen}")
    ...
    if out_filename is not None:
        ...
        for obj_id, filename in enumerate(combdataobj.filelist):
            ...
            for colid in range(nx_ori):
                ...
                print(f"[DEBUG] shapes for psf fit: {wvs_ori[:, colid].shape}, {wv_sampling.shape}, {all_interp_psfmodel[ny * obj_id:ny * (obj_id+1),  colid].shape}")
                ...
                new_area2d[:, colid] = np.interp(wvs_ori[:, colid], wv_sampling,
                                                 all_interp_area2d[ny * obj_id:ny * (obj_id+1),  colid], left=np.nan, right=np.nan)
...
def _build_cube_task_para(inputs):
    ...
    for ra_id, ra in enumerate(ra_vec):
        for dec_id, dec in enumerate(dec_vec):
            ...
            R = np.sqrt((X - ra) ** 2 + (Y - dec) ** 2)
            Zerr_masking = Zerr / median_abs_deviation(Zerr[np.where(np.isfinite(Zerr))])
            where_finite = np.where(np.isfinite(Zbp) * (Zerr_masking < 1e1) * np.isfinite(X) * np.isfinite(Y) * (R < aper_radius))
